...
class CreateActivityScreen extends StatefulWidget {
  ...
  @override State<CreateActivityScreen> createState() => _CreateActivityScreenState();
}
...
class _CreateActivityScreenState extends State<CreateActivityScreen> {
  ...
  Future<void> _publish() async {
      ...
      try {
        ...
        if (selectedDate.isBefore(DateTime.now())) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Cannot create an activity in the past.'),
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Activity published!')));
      context.pop();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: AppColors.cardSurface, title: const Text('Create Activity'), leading: IconButton(icon: const Icon(Icons.arrow_back_rounded), onPressed: () => context.pop())),
      body: SingleChildScrollView(
        ...
        child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_connectBooking) ...[
            ...
            SecondaryButton(label: 'Select Court', icon: Icons.search_rounded, onPressed: () => context.push('/booking')),
          ],
          const SizedBox(height: 28),
          PrimaryButton(label: 'Publish Activity', isLoading: prov.isLoading, onPressed: _publish),
