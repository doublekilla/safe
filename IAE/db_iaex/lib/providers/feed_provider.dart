...
class FeedProvider extends ChangeNotifier {
  ...
  Future<List<Map<String, dynamic>>> getComments(int postId) async {
    try {
      final res = await _api.get('/feed/$postId/comments');
      if (res.isSuccess && res.data is Map && res.data['data'] is Map && res.data['data']['data'] is List) {
        return List<Map<String, dynamic>>.from(res.data['data']['data']);
      } else if (res.isSuccess && res.data is Map && res.data['data'] is List) {
  ...
  Future<bool> addComment(int postId, String text) async {
    try {
      final res = await _api.post('/feed/$postId/comments', body: {'comment': text});
      if (res.isSuccess) {
        final index = _posts.indexWhere((p) => p.id == postId);
        if (index != -1) {
